...
Overview
************
The Fluid Run CI File is a YAML or json file in your repository that specifies the tests/benchmarks you want to run after building your code. Currently, this file consists of the a single list object :code:`tests` that has the following attributes :

* :code:`execution_command` This is the path to a script in your repository to run a specific test
* :code:`command_group` The command group is used to group execution commands that are dependent. Execution commands in the same command group are run sequentially in the order they are placed in the :code:`tests` block, unless the `--ignore-job-dependencies` flag is sent to fluid-run
